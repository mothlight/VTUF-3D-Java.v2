...
public class VegetationConfiguration
{
	...
	public static void main(String[] args)
	{
		...
		VegetationConfiguration treesData = new VegetationConfiguration();
		ArrayList<VegetationDataResults>  results = treesData.readVegetationHRWatDataFiles(1, 5, 1, rootDirectory);
		System.out.println(results.toString());

	}
	
	
	  public ArrayList<VegetationDataResults> readVegetationHRWatDataFiles(int treeConfigLocation, double width1, int diffShadingType, String rootDirectory)
	    {
	    ...
	    else
	    {
	    	...
	    	VegetationDataFile WATBALDAT_FILE = new VegetationDataFile(watbalfilestr);
	    	VegetationDataFile HRFLXDAT_FILE = new VegetationDataFile(hrflxfilestr);
	        ...
	        ArrayList<VegetationDataResults> vegetationDataForTimesteps = new ArrayList<VegetationDataResults>();
//	        System.out.println(treeConfigLocation + " " + diffShadingType);
	        for (int i=0;i<hrTHM.length;i++)
	        {
	        	VegetationDataResults item = new VegetationDataResults();
...
//		i + " " + et[i+1] + " " + soilevap[i+1] + " " + canopystore[i+1] + " " + evapstore[i+1] + " " + item.qeCalc5);	        	
	        	vegetationDataForTimesteps.add(item);
	        	
	        }
  
	        return vegetationDataForTimesteps;
	    }

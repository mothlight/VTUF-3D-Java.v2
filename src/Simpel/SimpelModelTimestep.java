...
//		String[] InputStr2 = new String[] {"20.1.2021.1","20","0","59.315","14.3271666666667","0.443066666666667","0","0"};
	...
	public void getDailyEPT(int Day, double[] Ktotfrc, double[] ea, double[] Ta)
	{
		double[] returnEPT = new double[24];
		ETo eto = new ETo();						        		
//		The latitude of the met station (dec deg) 
		double lat=-37.5;
//		The longitude of the met station (dec deg) (only needed if calculating ETo hourly)
...
//			Incoming shortwave radiation (MJ/m2)
			double R_s_hourly = Ktotfrc[hour] * 60. * 60. * 1E-6;  
//			Actual Vapour pressure derrived from RH
			double e_a_hourly = ea[hour];  
//			Mean Temperature (deg C)
			double T_mean_hourly = Ta[hour];		
			double etoValue = eto.eto_fao_hourly(freq, lat, Day, lon, TZ_lon, z_msl, e_a_hourly, R_s_hourly, T_mean_hourly, z_u, U_z, alb, hour);
			returnEPT[hour]=etoValue;
		}
		
	}
	
//	public void runDailySimpel()
//	{
...
//		  # water balance check
		  double water_balance = sum_prec - sum_etr - sum_runoff + init_swe + init_stor - bucket_model[krowEnd][SimpelConstants.STORAGE] 
				  - bucket_model[krowEnd][SimpelConstants.SNOW_WATER_EQUI] - bucket_model[krowEnd][SimpelConstants.I_BAL] 
						  - bucket_model[krowEnd][SimpelConstants.CONTENT];
...
//		  # water balance check
	...
	public double CalculateLatentHeat(double Tair)
	{
//	    #calculate latent heat of vaporization dependent on air temperature

//	    if len(Mod.time) == sum(~np.isnan(Mod.Tair.values)).item():

	     double   L = 4185.5 * (751.78 - 0.5655 * Tair)/1000;

//	    #take 20 degrees if no temperature data is available

//	    else:
//
//	        L = float(2452.718);
//
////	    #add latent heat of fusion in case of a snowpack
//
//	    if ('SWE' in list(Mod.keys())) & ('SnowFrac' in list(Mod.keys())):
//
//	        Mod['SnowFrac'] = Mod['SnowFrac'].fillna(0);
//
//	        Mod['SWE'] = Mod['SWE'].fillna(0);
//
//	        L = xr.where(Mod['SWE']>0.1, L + (334 * Mod['SnowFrac']), L);
//
//	    elif 'SnowFrac' in list(Mod.keys()):
//
//	        Mod['SnowFrac'] = Mod['SnowFrac'].fillna(0);
//
//	        L += Mod['SnowFrac'] * 334;
...
//
//	        if sum(~np.isnan(Mod.Qle.values)).item()==len(Mod.time):
//
//	            L=np.average(L, weights=Mod.Qle).item();
//
//	        elif 'Evap' in Mod.keys():
//
//	            if sum(~np.isnan(Mod.Evap.values)).item()==len(Mod.time):
//
//	                L=np.average(L, weights=Mod.Evap).item();
//
//	        else:
//
//	            L=np.average(L).item();
